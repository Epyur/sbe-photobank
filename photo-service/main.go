...
func main() {
	...
	// «ЦУП Веб» (2026-09-02): requireNotWebChannel блокирует запись для веб-сессий
	// (channel="web") независимо от роли — веб-порталу доступны только просмотр/
	// поиск/соцслой (лайк/избранное/комментарии остаются без обёртки ниже).
	mux.HandleFunc("POST /api/photo/file", s.requirePerm("editor")(requireNotWebChannel(s.handleUploadFile)))
	mux.HandleFunc("POST /api/photo/thumb", s.requirePerm("editor")(requireNotWebChannel(s.handleUploadThumb)))
	...
	mux.HandleFunc("POST /api/photo/folders", s.requirePerm("admin")(requireNotWebChannel(s.handleCreateFolder)))
	mux.HandleFunc("POST /api/photo/folders/rename", s.requirePerm("admin")(requireNotWebChannel(s.handleRenameFolder)))
	mux.HandleFunc("POST /api/photo/folders/move", s.requirePerm("viewer")(requireNotWebChannel(s.handleMoveFolder)))
	mux.HandleFunc("DELETE /api/photo/folders/{id}", s.requirePerm("viewer")(requireNotWebChannel(s.handleDeleteFolder)))
	mux.HandleFunc("POST /api/photo/folders/{id}/limited", s.requirePerm("viewer")(requireNotWebChannel(s.handleSetFolderLimited)))
	mux.HandleFunc("GET /api/photo/folders/{id}/permissions", s.requirePerm("viewer")(s.handleListFolderPerms))
	mux.HandleFunc("POST /api/photo/folders/{id}/permissions", s.requirePerm("viewer")(requireNotWebChannel(s.handleSetFolderPerm)))
	mux.HandleFunc("GET /api/photo/permissions", s.requirePerm("admin")(requireNotWebChannel(s.handleListPermissions)))
	mux.HandleFunc("POST /api/photo/permissions", s.requirePerm("admin")(requireNotWebChannel(s.handleSetPermission)))
	mux.HandleFunc("GET /api/photo/permissions/me", s.requirePerm("viewer")(s.handleMyPermission))
	mux.HandleFunc("GET /api/photo/common-access", s.requirePerm("admin")(requireNotWebChannel(s.handleGetCommonAccess)))
	mux.HandleFunc("POST /api/photo/common-access", s.requirePerm("admin")(requireNotWebChannel(s.handleSetCommonAccess)))
	mux.HandleFunc("GET /api/photo/groups", s.requirePerm("viewer")(s.handleListGroups))
	mux.HandleFunc("POST /api/photo/groups", s.requirePerm("admin")(requireNotWebChannel(s.handleSaveGroup)))
	mux.HandleFunc("DELETE /api/photo/groups/{id}", s.requirePerm("admin")(requireNotWebChannel(s.handleDeleteGroup)))
	mux.HandleFunc("GET /api/photo/schema", s.requirePerm("viewer")(s.handleGetSchema))
	mux.HandleFunc("POST /api/photo/schema", s.requirePerm("admin")(requireNotWebChannel(s.handleSetSchema)))
	mux.HandleFunc("GET /api/photo/photos/{id}", s.requirePerm("viewer")(s.handleGetPhoto))
	mux.HandleFunc("DELETE /api/photo/photos/{id}", s.requirePerm("viewer")(requireNotWebChannel(s.handleDeletePhoto)))
	...
	mux.HandleFunc("GET /api/photo/photos/{id}/visibility", s.requirePerm("admin")(requireNotWebChannel(s.handleGetVisibilityOverride)))
	mux.HandleFunc("POST /api/photo/photos/{id}/visibility", s.requirePerm("admin")(requireNotWebChannel(s.handleSetVisibilityOverride)))
