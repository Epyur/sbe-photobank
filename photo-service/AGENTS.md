...
- **2026-09-03 — «Просмотр от лица роли» (веб), по прямому запросу пользователя.**
  Заголовок `X-View-As-Role: viewer|commenter|editor|admin` — только для
  реального `superadmin`, только `channel=web`, только на роль строго ниже
  `superadmin` (в photo-service клэмпа superadmin→admin для web нет — запись
  и так полностью блокируется `requireNotWebChannel` независимо от роли,
  подмена здесь влияет только на видимость). Реализовано в `effectiveRole`
  (`jwt.go`): сначала `rawRole()` (реальная роль, без подмены — используется
  в `handleMyPermission` как `real_role`, иначе суперадмин, включивший
  просмотр как viewer, терял бы сам переключатель), затем подмена, если
  условия выполнены. Проверяется на каждый вызов по реальной роли из БД —
  ограничивает видимость файлов/папок везде, где читается `effectiveRole`,
