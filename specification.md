...
- **ИИ-описание при загрузке**: пользователь даёт контекст (что на кадре, событие, локация,
  ...
  `{title, description, tags[], category?, location?, shot_at?, custom{...}}`.
  Vision у sbe-llm нет — описание строится из контекста + имени файла + папки.
  Если sbe-llm недоступен — ручное заполнение (graceful degradation).
