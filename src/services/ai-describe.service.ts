...
export class AiDescribeService {
  ...
  async describe(input: {
    ...
    const systemVision = 'Ты — помощник по каталогизации корпоративного сток-фотобанка. ' +
      ...
      'СТРОГОЕ ПРАВИЛО: НЕ фантазируй и не додумывай — описывай ТОЛЬКО то, что реально видно на изображении. ' +
      'Не выдумывай объекты, детали, подписи, текст или сюжет, которых на фото нет. Если не уверен — не упоминай. ' +
      'Путь в банке и имя файла даны для контекста, но содержимое описания берётся ТОЛЬКО с картинки. ' +
      'ПРАВИЛО ИМЕНИ: «title» не должен содержать техническое имя файла (например "image67", "IMG_1234", "DSC_0001", "photo_2026") ' +
      'и не должен повторять его дословно — придумай осмысленное короткое название по содержимому кадра. ' +
    ...
    const systemText = 'Ты — помощник по каталогизации корпоративного сток-фотобанка. ' +
      'По контексту пользователя, имени файла и пути в банке составь краткое, точное расширенное описание ' +
      'и метаданные для поиска. В описание включи, что, судя по контексту, находится на кадре, композицию ' +
      'и цветовую схему, а также любые детали, которые упомянул пользователь. ' +
      'СТРОГОЕ ПРАВИЛО: НЕ выдумывай детали, объекты, цвета и сюжет, которых нет в контексте пользователя ' +
      'и не следует из имени файла или названия папки. Если контекст пуст — опиши только то, что следует ' +
      'из имени файла и пути, и пометь неуверенность. Не фантазируй. ' +
      'ПРАВИЛО ИМЕНИ: «title» не должен содержать техническое имя файла (например "image67", "IMG_1234", "DSC_0001", "photo_2026") ' +
      'и не должен повторять его дословно — придумай осмысленное короткое название по контексту и имени файла. ' +
      'Текст на русском. Верни ТОЛЬКО JSON без пояснений: ' +
      '{"title": "ОТОБРАЖАЕМОЕ В СТОКЕ ИМЯ файла — короткое, ёмкое, продающее (≤60 симв.), по которому файл показывается в сетке и поиске", ' +
